...
Vec3 Triangle::ApproxPosition() restrict(amp) {
	return (Corners[0]+Corners[1]+Corners[2])*(1.0f/3);
}

float Triangle::CorrectDistance(Ray r, float d) restrict(amp) {
	return (r.Origin - IntersectionPoint(r, d)).norm() / r.Origin.norm();
}

bool Triangle::RayHit(Ray r) restrict(amp) {
	return RayHitDistance(r) != -1;
}


float Triangle::RayHitDistance(Ray r) restrict(amp) {
...
Ray Triangle::PointNormal(Vec3 Point, Ray oRay) restrict(amp) {
	Ray nRay;
